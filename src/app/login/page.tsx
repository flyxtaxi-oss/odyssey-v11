...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setMessage(null);
        try {
            const result: AuthResult = mode === "login" ? await signIn(email, password) : await signUp(email, password);
            if (result.error) {
    ...
    return (
        <div
            ...
            >
                <Card variant="glass" padding="none" className="p-10 sm:p-12">
                    {/* Logo */}
                    <div className="text-center mb-9">
                        ...
                        <h1 className="text-[28px] font-extrabold font-display tracking-tight text-[var(--text-0)]">
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                        ...
                        <label className="flex flex-col gap-2 text-sm">
                            <span className="text-xs font-semibold uppercase tracking-wider text-[var(--text-2)]">Email</span>
                            <div className="relative">
                                <Mail size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--text-3)] pointer-events-none" />
                                <Input
                                    type="email"
                                    value={email}
                        ...
                        {/* Password */}
                        <label className="flex flex-col gap-2 text-sm">
                            <span className="text-xs font-semibold uppercase tracking-wider text-[var(--text-2)]">Mot de passe</span>
                            <div className="relative">
                                <Lock size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--text-3)] pointer-events-none" />
                                <Input
                                    ...
                                    minLength={6}
                                    disabled={isLoading}
                                    className="py-3.5 pl-11 pr-11 text-[15px]"
                                />
                                <button
                                    type="button"
                            ...
                            {mode === "signup" && (
                                <span className="text-[11px] text-[var(--text-3)]">Minimum 6 caractères</span>
                            )}
                        </label>

                        {/* Submit — preserves the auth gradient identity, via the design-system Button */}
                        <Button
                            type="submit"
                            size="lg"
                            loading={isLoading}
                            className="w-full mt-1 text-white bg-[linear-gradient(135deg,#2563EB,#7C3AED)] hover:shadow-[0_0_28px_rgba(124,58,237,0.35)]"
                        >
                            {!isLoading && (
                                <>
                        ...
                        <div className="flex items-center gap-3 my-1">
                            <div className="flex-1 h-px bg-[var(--border-0)]" />
                            <span className="text-xs font-semibold uppercase tracking-wider text-[var(--text-3)]">ou</span>
                            <div className="flex-1 h-px bg-[var(--border-0)]" />
                        </div>
                    ...
                    <p className="text-center mt-6 text-sm text-[var(--text-2)]">
                        ...
                        <button
                            onClick={toggleMode}
                            disabled={isLoading}
                            className="font-semibold text-sm text-[#2563EB] hover:underline disabled:opacity-70"
                        >
